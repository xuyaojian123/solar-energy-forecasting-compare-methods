...
class ANN_Model(nn.Module):
    def __init__(self, settings):
        ...
        self.nn = torch.nn.Sequential(
            nn.Linear(self.input_len * self.input_size, 200),
            nn.ReLU(),
            nn.Linear(200, 100),
            torch.nn.ReLU(),
            nn.Linear(100, 50),
            torch.nn.ReLU(),
            nn.Linear(50, self.output_len)
        )
...
def forecast_ANN(settings):
    model_filenames = ['ANN_o_15.pt', 'ANN_o_45.pt', 'ANN_o_90.pt', 'ANN_o_150.pt']
...
if __name__ == "__main__":
    ...
    for horizon in settings['horizons']:
        ...
        model = ANN_Model(settings).to(device)
        ...
        torch.save(model, settings['checkpoints'] + f"ANN_o_{horizon}.pt")
        torch.cuda.empty_cache()

    
    # forecast future solar energy
    gru_forecast_results = forecast_ANN(settings)
    ...
    for i in range(len(gru_forecast_results)):
        ...
        plt.legend()
        plt.savefig('./results/ANN/predicted_vs_true_values' + str(i) + '.png')
        ...
        predictions.to_csv(f"./results/ANN/prediction_length_{L}.csv", index=False)
